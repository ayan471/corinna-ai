...
const CodeSnippet = ({ id }: Props) => {
  ...
  let snippet = `
    ...
    iframe.src = "https://corinna-ai-ayan471s-projects.vercel.app/chatbot"
    iframe.classList.add('chat-frame')
    document.body.appendChild(iframe)
    
    window.addEventListener("message", (e) => {
        if(e.origin !== "https://corinna-ai-ayan471s-projects.vercel.app") return null
        let dimensions = JSON.parse(e.data)
        iframe.width = dimensions.width
        iframe.height = dimensions.height
        iframe.contentWindow.postMessage("${id}", "https://corinna-ai-ayan471s-projects.vercel.app/")
    })
        `;
